...
namespace BasePluginTests.Infrastructure
{
    public class PluginServiceResolverTests
    {
        ...
        private readonly Mock<IOrganizationService> _executionService;
        private readonly Mock<IOrganizationService> _permissionCheckService;
        ...
        public PluginServiceResolverTests()
        {
            ...
            _executionService = new Mock<IOrganizationService>();
            _permissionCheckService = new Mock<IOrganizationService>();
            ...
            _context.Setup(c => c.InitiatingUserId).Returns(Guid.NewGuid());
            _factory
                .Setup(f => f.CreateOrganizationService(It.IsAny<Guid?>()))
                .Returns<Guid?>(userId =>
                    userId.HasValue
                    && userId.Value == _context.Object.InitiatingUserId
                    && userId.Value != _context.Object.UserId
                        ? _permissionCheckService.Object
                        : _executionService.Object
                );
        }

        [Fact]
        public void Resolve_returns_services_when_identities_differ()
        {
            var userId = Guid.NewGuid();
            var initiatingUserId = Guid.NewGuid();
            _context.Setup(c => c.UserId).Returns(userId);
            _context.Setup(c => c.InitiatingUserId).Returns(initiatingUserId);
            ...
            Assert.Same(_executionService.Object, services.ExecutionService);
            Assert.Same(_permissionCheckService.Object, services.PermissionCheckService);
            _factory.Verify(f => f.CreateOrganizationService(userId), Times.Once);
            _factory.Verify(f => f.CreateOrganizationService(initiatingUserId), Times.Once);
        }

        [Fact]
        public void Resolve_reuses_execution_service_when_identities_match()
        {
            var userId = Guid.NewGuid();
            _context.Setup(c => c.UserId).Returns(userId);
            _context.Setup(c => c.InitiatingUserId).Returns(userId);

            var services = PluginServiceResolver.Resolve(_serviceProvider.Object);

            Assert.Same(_executionService.Object, services.ExecutionService);
            Assert.Same(_executionService.Object, services.PermissionCheckService);
            _factory.Verify(f => f.CreateOrganizationService(userId), Times.Once);
        }
        ...
        public void Resolve_traces_and_throws_when_service_factory_missing()
        {
            ...
            _tracing.Verify(
                t => t.Trace("Organization service factory is unavailable."),
                Times.Once
            );
        }
    }
